/* skip999
 * 5/31/2020
 * purpose: to house methods for DemoicAlloyForge functionality
 */
...
public class DemonicAlloyForge extends Block{
...
/* Fields */
	...
	public DemonicAlloyForge(Properties properties) {
		super(properties);
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return BlockTileEntityTypes.DEMONIC_ALLOY_FORGE.get().create();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		if(stack.hasDisplayName()) {
			...
			if(tile instanceof DemonicAlloyForgeTileEntity) {
				((DemonicAlloyForgeTileEntity)tile).setCustomName(stack.getDisplayName());
			}
		}
	}
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		TileEntity tile = worldIn.getTileEntity(pos);
		if (tile instanceof DemonicAlloyForgeTileEntity && state.getBlock() != newState.getBlock()) {
			DemonicAlloyForgeTileEntity demonicAlloyFurnace = (DemonicAlloyForgeTileEntity) tile;
			((TileEntityItemHandler) demonicAlloyFurnace.getInventory()).toNonNullList().forEach(item -> {
