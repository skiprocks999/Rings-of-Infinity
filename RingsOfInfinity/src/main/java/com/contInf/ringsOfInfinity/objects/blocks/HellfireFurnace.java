/* skip999
 * 5/31/2020
 * purpose: to house methods for HellfireFurnace
 */
...
public class HellfireFurnace extends Block {
...
/* Fields */
	...
	public HellfireFurnace(Properties properties) {
		super(properties);
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return BlockTileEntityTypes.HELLFIRE_FURNACE.get().create();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		if(stack.hasDisplayName()) {
			...
			if(tile instanceof HellfireFurnaceTileEntity) {
				((HellfireFurnaceTileEntity)tile).setCustomName(stack.getDisplayName());
			}
		}
	}
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		TileEntity tile = worldIn.getTileEntity(pos);
		if (tile instanceof HellfireFurnaceTileEntity && state.getBlock() != newState.getBlock()) {
			HellfireFurnaceTileEntity hellfireFurnace = (HellfireFurnaceTileEntity) tile;
			((TileEntityItemHandler) hellfireFurnace.getInventory()).toNonNullList().forEach(item -> {
