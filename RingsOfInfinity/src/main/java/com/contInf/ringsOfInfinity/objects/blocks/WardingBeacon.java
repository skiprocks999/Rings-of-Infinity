/* skip999
 * 5/23/2020
 * purpose: contain functionality methods for WardingBeacon block
 */
...
public class WardingBeacon extends Block implements IBeaconBeamColorProvider{
	...
	public WardingBeacon(Properties properties) {
		super(properties);
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return BlockTileEntityTypes.WARDING_BEACON.get().create();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		if(stack.hasDisplayName()) {
			...
			if(tile instanceof WardingBeaconTileEntity) {
				((WardingBeaconTileEntity)tile).setCustomName(stack.getDisplayName());
			}
		}
	}
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		TileEntity tile = worldIn.getTileEntity(pos);
		if (tile instanceof WardingBeaconTileEntity && state.getBlock() != newState.getBlock()) {
			WardingBeaconTileEntity wardingBeacon = (WardingBeaconTileEntity) tile;
			((TileEntityItemHandler) wardingBeacon.getInventory()).toNonNullList().forEach(item -> {
