...
public class DemonicWardingBeaconScreen extends ContainerScreen<DemonicWardingBeaconContainer>{

	/* Fields */
	private static final ResourceLocation TEXTURE = new ResourceLocation(RingsOfInfinity.modID,
			"textures/gui/demonic_warding_beacon.png");
	
	private static final Logger logger = LogManager.getLogger(RingsOfInfinity.modID);
	
	/* Constructors */

	public DemonicWardingBeaconScreen(DemonicWardingBeaconContainer screenContainer, PlayerInventory inv,
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
		...
		DemonicWardingBeaconScreen.blit(matrixStack,this.guiLeft, this.guiTop, 0, 0, this.xSize, this.ySize,256,256);
		
		//blit(int x, int y, int textureX,
		//					int textureY, int width, int height, int textureWidth, int textureHeight);

		//Smelt Time Bar
		this.blit(matrixStack,this.guiLeft + 25, this.guiTop + 8, 0, 166, 
				this.container.getBurnProgressionScaled(), 68);
	}
